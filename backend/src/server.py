...
def receive_data():
    ...
    package_id = f"{data['ip']}-{data['timestamp']}-{data['latitude']}-{data['longitude']}"
    if package_id not in unique_packages:
...
def get_stats():
    ...
    return jsonify({
        ...
        'top_locations': [
            {'lat': loc[0], 'lng': loc[1], 'count': count}
            for loc, count in sorted(location_counts.items(), 
                                   key=lambda x: x[1], 
                                   reverse=True)[:5]
        ],
        'continents': [
            {'continent': cont, 'count': count}
            for cont, count in sorted(activity_data.items(),
                                    key=lambda x: x[1],
                                    reverse=True)
        ]
    })
